...
DEPT_SEASONS_DEF        = "%s/Season/Auto/{client_id}/?h_id={h_id}" % (HOST)
DEPT_SEASON_LINK        = "%s/Season/Auto/{client_id}/?h_id={h_id}" % (HOST)
...
#=======================================================================================================================
#mm
def v2_get_data(to_date, client_id, hotel_id, year_back=4):
...
def get_data_for_dep_season(client_id, hotel_id, year_back=4):
    to_date = datetime.now().date()
    year_add = - year_back
    from_date = add_years(to_date, year_add)
...
def get_season_data(client_id, dept_id):
    ...
    del unpack_df['Dates']
    df_merge = pd.merge(unpack_df, df_new, on='Color', how='left')
    return df_merge
...
class Cruise_day:
    def __init__(self, to_date, client_id, hotel_id, year_back = 4):
        self.get_cruise_day(to_date, client_id, hotel_id, year_back)

    def get_cruise_day(self, to_date, client_id, hotel_id, year_back = 4):
#        to_date = datetime.now().date()
        year_add = - year_back
        from_date = add_years(to_date, year_add)
