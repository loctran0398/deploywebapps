...
class Cruise_day:
    def __init__(self, to_date, df, hotel_id, year_back = 2):
        self.get_cruise_day(to_date, df, hotel_id, year_back)

    def get_cruise_day(self, to_date, df, hotel_id, year_back = 2):
#        to_date = datetime.now().date()
        year_add = - year_back
        from_date = add_years(to_date, year_add)
#        link = CRUISE_DAY.format(client_id=client_id, h_id=hotel_id, from_date=str(from_date), to_date=str(to_date))
#        self.__cruise_day = request_data(link)
        self.__cruise_day = df.copy()
        self.__cruise_day_copy = self.__cruise_day.copy()

        self.__cruise_day = self.__cruise_day[self.__cruise_day['Period_Type'] != 2].reset_index(drop = True)
